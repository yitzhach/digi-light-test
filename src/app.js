...
function loadFile(file) {
  return new Promise((resolve, reject) => {
    ...
    img.onerror = () => { URL.revokeObjectURL(url); reject(new Error('Could not decode that image. Please use JPEG, PNG or WebP.')); };
    img.src = url;
  });
}
...
async function setSource(src) {
  ...
  const MAX = Math.min(1400, glctx.caps.maxTexture);
...
function rebuildTabs() {
  ...
  state.lights.forEach((l, i) => {
    ...
    b.onclick = () => { state.selected = i; rebuildTabs(); rebuildLightPanel(); rebuildHandles(); };
    tabs.appendChild(b);
  });
...
function rebuildViews() {
  ...
  VIEWS.forEach(([name, mode, note]) => {
    const b = document.createElement('button');
    b.textContent = name;
    if (mode === 5 && state.mode !== 'photometric') return;
...
function renderNow() {
  ...
  try {
      ...
      if (state.mode === 'photometric') {
        ...
        if (solver.dof <= 0) {
          ...
          if (fit && status) {
            ...
            const verdict = fit.mean < 0.015 ? 'good'
              : fit.mean < 0.05 ? 'usable' : 'poor — check alignment and light angles';
    ...
    if (state.viewMode === 5) {
      ...
      else shader.draw(drawTargets, state, imgH / imgW, imgW, imgH);
    } else {
...
async function boot() {
  ...
  $('openPhoto').onclick = () => { $('src').value = 'upload'; $('src').dispatchEvent(new Event('change')); $('file').click(); };
  $('src').addEventListener('change', async () => {
    ...
    $('surfaceControls').style.opacity = photometric ? 0.35 : 1;
    updateChromaNote();

    state.viewMode = 0; state.psShowTruth = false;
    if (v === 'pupload') { disposeShots(); state.sphere = null; drawSphereOverlay(); $('psStatus').textContent = 'Choose at least 4 matching photographs; set the light angles for each.'; }
    $('psTruth').className = '';
    rebuildViews();
    dirtySurface = true;
...
function wireExport() {
  ...
  btn.addEventListener('click', async () => {
    ...
    if (state.mode === 'photometric' && !buildSolver(shots.map(shotDir), { fitAmbient: state.psFitAmbient }).ok) { status.textContent = 'Cannot export: check capture angles and shot count.'; return; }
    btn.disabled = true;
    document.querySelectorAll('#panel input, #panel select, #panel button').forEach(el => { el.dataset.exportDisabled = String(el.disabled); el.disabled = true; });
...
function packShots(w, h) {
  ...
  const scale = Math.min(1, 1400 / Math.max(w, h), glctx.caps.maxTexture / Math.max(w, h));
  const pw = Math.max(1, Math.round(w * scale)), ph = Math.max(1, Math.round(h * scale));
  shotArrayTex = uploadShotArray(gl, shots.map((s2) => {
    const c = document.createElement('canvas'); c.width = pw; c.height = ph;
    c.getContext('2d').drawImage(s2.source, 0, 0, pw, ph); return c;
  }), pw, ph);
}
...
async function loadUploadedCapture(files) {
  ...
  disposeShots();
  state.sphere = null; state.psShowTruth = false; $('psTruth').className = ''; drawSphereOverlay();
  shots = imgs.map((im, i) => ({
...
boot().catch(fail);
canvas.addEventListener('webglcontextlost', (event) => { event.preventDefault(); fail(new Error('Graphics context lost. Reload the page and reopen your photograph.')); });
